...
const App = () => {
  ...
  const HabitTracker = () => (
    <div className="card full-width" title="List of habits you want to maintain daily">
      ...
      <div className="habit-list">
        {habits.map(h => (
          <div key={h.id} className="habit-item" title={`Click checkbox to complete ${h.name}`}>
            ...
            <button className="delete-icon" onClick={() => deleteHabit(h.id)} title="Delete this habit"><Trash2 size={18} /></button>
          </div>
        ))}
  ...
  const BMICard = () => (
    <div className="card full-width" title="Monthly check-in for Body Mass Index">
      <h3 className="card-title">Monthly BMI Check</h3>
      <div className="bmi-flex">
        <form className="bmi-form" onSubmit={calculateBMI}>
          <input name="weight" type="number" step="0.1" placeholder="Weight (kg)" className="todo-input" required title="Enter your weight in kilograms" />
          <input name="height" type="number" placeholder="Height (cm)" className="todo-input" required title="Enter your height in centimeters" />
          <button type="submit" className="btn-primary" style={{ justifyContent: 'center' }} title="Calculate BMI score">Analyze Results</button>
        </form>
  ...
  return (
    <div className={`app-container ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <aside className="sidebar">
        <div className="brand" title="HabitFlow Health Dashboard">
          <div className="brand-icon"><TrendingUp size={24} /></div>
          <span className="brand-name">HabitFlow</span>
        </div>
        
        <nav className="nav-list">
          <button className={`nav-item ${activeTab === 'dashboard' ? 'active' : ''}`} onClick={() => setActiveTab('dashboard')} title="Dashboard Overview"><Layout size={20} /><span>Dashboard</span></button>
          <button className={`nav-item ${activeTab === 'habits' ? 'active' : ''}`} onClick={() => setActiveTab('habits')} title="My Habits"><Check size={20} /><span>Habits</span></button>
          <button className={`nav-item ${activeTab === 'sleep' ? 'active' : ''}`} onClick={() => setActiveTab('sleep')} title="Sleep Analysis"><Moon size={20} /><span>Sleep</span></button>
          <button className={`nav-item ${activeTab === 'bmi' ? 'active' : ''}`} onClick={() => setActiveTab('bmi')} title="BMI Tracker"><Scale size={20} /><span>BMI Logs</span></button>
          <button className={`nav-item ${activeTab === 'settings' ? 'active' : ''}`} onClick={() => setActiveTab('settings')} title="Settings & Theme"><Settings size={20} /><span>Settings</span></button>
        </nav>
      </aside>
      ...
      <main className="main-content">
        <header className="page-header">
          <div>
            <span className="header-meta">{darkMode ? 'Evening' : 'Good Morning'}, Tracker</span>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
             <button className="btn-primary" style={{ background: darkMode ? '#334155' : '#f1f5f9', color: 'var(--text-main)', boxShadow: 'none' }} onClick={() => setDarkMode(!darkMode)} title="Toggle Dark Mode">
               {darkMode ? <Sun size={20} /> : <Moon size={20} />}
             </button>
             <button className="btn-primary" onClick={() => setIsModalOpen(true)} title="Add new goal"><Plus size={20} /> <span>New Habit</span></button>
          </div>
        </header>
        ...
        {activeTab === 'dashboard' && (
          <>
            ...
            <div className="dashboard-grid">
              ...
              <SleepChart />
              <div className="card" title="Notes and quick reminders">
                <h3 className="card-title">Quick Notes</h3>
                <div className="todo-input-wrap"><input type="text" className="todo-input" placeholder="Buy supplements..." value={newTodo} onChange={e => setNewTodo(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTodo()} /><button className="btn-primary" style={{ padding: '0.6rem' }} onClick={addTodo}><Plus size={18} /></button></div>
                <div className="todo-list">{todos.map((t, idx) => (<div key={idx} className="todo-item"><span>• {t}</span><button className="delete-icon" onClick={() => deleteTodo(idx)}><X size={14} /></button></div>))}</div>
              </div>
              <BMICard />
              
              {/* Settings Button Card below BMI */}
              <div className="card full-width" style={{ marginTop: '1rem', background: darkMode ? 'var(--primary)' : '#eff6ff', border: 'none', cursor: 'pointer' }} onClick={() => setActiveTab('settings')} title="Manage your progress and theme preferences">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div style={{ background: 'white', padding: '8px', borderRadius: '10px', color: 'var(--primary)' }}><Settings size={20} /></div>
                    <div>
                      <div style={{ fontWeight: 800, color: darkMode ? 'white' : 'var(--primary)' }}>Configuration & Theme</div>
                      <div style={{ fontSize: '0.8rem', color: darkMode ? '#ffffff99' : '#1e3a8a99' }}>Toggle Dark Mode, access Monthly Reports, and set reminders</div>
                    </div>
                  </div>
      ...
      {isModalOpen && (
        <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}><button className="nav-item" style={{ justifyContent: 'center', background: 'var(--bg)' }} onClick={() => setIsModalOpen(false)}>Cancel</button><button className="btn-primary" style={{ flex: 1, justifyContent: 'center' }} onClick={addHabit}>Start Tracking</button></div>
          </div>
        </div>
      )}
    </div>
